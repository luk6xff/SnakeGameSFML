...
int lWidth, sf::Vector2f l_screenPos){
	Setup(l_visible, l_charSize, lWidth, l_screenPos);
}
...
int lWidth, sf::Vector2f l_screenPos){
	...
	mBackdrop.setSize(sf::Vector2f(
		lWidth, (l_visible * (l_charSize * 1.2f))));
	mBackdrop.setFillColor(sf::Color(90,90,90,90));
	mBackdrop.setPosition(l_screenPos);
}
